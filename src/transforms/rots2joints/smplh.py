...
from src.info.joints import smplh_to_mmm_scaling_factor
from src.info.joints import smplh2mmm_indexes
import hydra
...
class SMPLH(Rots2Joints):
    def __init__(self, path: str,
                 jointstype: str = "mmm", # TODO change it to smplh
        ...
        from smplx.body_models import SMPLLayer, SMPLHLayer
        import os
        rel_p = path.split('/')
        rel_p = rel_p[rel_p.index('data'):]
        rel_p = '/'.join(rel_p)
        path = hydra.utils.get_original_cwd() + '/' + rel_p
        markers_path = hydra.utils.get_original_cwd() + '/data/smpl_models/markers_mosh/smplh/SSM67.json'
        if not fast:
            # Remove annoying print
            with contextlib.redirect_stdout(None):
                self.smpl = SMPLHLayer(path, ext="pkl", 
                                    gender=gender)#.eval()#.to(device='cuda').eval()

            self.faces = self.smpl.faces
            for p in self.parameters():
                p.requires_grad = False
        else:
            from src.transforms.rots2joints.smpl_fast import SMPLIndexed
            from src.utils.file_io import read_json
            markers_dict = read_json(markers_path)['markersets'][0]['indices']
            marker_ssm67 = list(markers_dict.values())
            # with open() as f:
            #     marker_ssm67 = list(json.load(f)['markersets'][0]['indices'].values())

            with contextlib.redirect_stdout(None):
                self.smpl = SMPLIndexed(path,
                                        model_type='smpl',
                                        gender=gender,
                                        ext="pkl",
                                        extra_joint_module_type='from_transforms',
                                        lmk_obj_type='from_transforms',
                                        vertex_indices=marker_ssm67,
                                        optim_transform=True)#.eval()#to(device='cuda')#.eval()
            self.faces = self.smpl.faces
            for p in self.parameters():
                p.requires_grad = False
    ...
    def forward(self, smpl_data: dict,
        ...
        poses = poses.to(self.smpl.posedirs.device)
        trans = trans.to(self.smpl.posedirs.device)
        ...
        matrix_poses = poses
        # matrix_poses = F.pad(input=poses, pad=(0, 0, 0, 0, 0, 2, 0, 0), 
        #                      mode='constant',
        #                      value=0)
        ...
        for chunk in range(int((n - 1) / batch_size) + 1):
            ...
            smpl_output = self.smpl(global_orient=slice_or_none(global_orient, chunk_slice),
...
def smplh_to(jointstype, data, trans):
    from src.info.joints import get_root_idx
