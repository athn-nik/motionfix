...
class MultiActorAgnosticEncoder(pl.LightningModule):
    ...
    def forward(self, features: Tensor,
                lengths: Optional[List[int]] = None) -> Union[Tensor, Distribution]:
        if lengths is None:
        ...
        if self.hparams.vae:
            ...
            token_mask = torch.ones((bs, 2*self.z_len), dtype=bool, device=x.device)
